    ...
    if (typeof this.append !== "function") {
        LinkList.prototype.append = function (node) {
            ...
            if (this.head == null) {
                ...
                node.index = _nodesDD.length+12; //务必在push前设置node.index
                _nodesDD.push(node);
...
function SingleList() {
    ...
    if (typeof this.append !== "function") {
        SingleList.prototype.append = function (node) {
            ...
            node.index = _nodesDD.length; //务必在push前设置node.index
            _nodesDD.push(node);
        }
    }
}
    ...
    _carousel.bindTouchEvent = function () {
        ...
        _container.addEventListener("touchend",function(e) {
            ...
            if (isMove) { //发生了左右滑动
                ...
                var obj = e.currentTarget.getElementsByTagName("li")[_current.index - 12];
                var url = obj.getElementsByTagName("a")[1].attributes['href'].nodeValue;
                location.href = url;
            }
            time1 = setInterval(imgAutoMove, 3000);
        }, false);
