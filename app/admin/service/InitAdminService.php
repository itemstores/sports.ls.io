...
use app\admin\model\SportsCate;
use app\admin\model\SportsWorldcup;
use app\admin\model\SportsTag;
...
class InitAdminService
{
    ...
    public function initSports()
    {
        $output = $this->output;
        
        $output->writeln('开始初始化体育案例');
        
        $list_cate = $this->requireData('SportsCate');
        
        $list_worldcup = $this->requireData('SportsWorldcup');
        
        foreach ($list_cate as  $data_cate) {
            $model_cate = SportsCate::create($data_cate);
            
            foreach ($list_worldcup as  $data_worldcup) {
                $data_worldcup['cate_id'] = $model_cate->id;
                SportsWorldcup::create($data_worldcup);
            }
        }
        
        $list_tag = $this->requireData('SportsTag');
        
        $this->installData(SportsTag::class, $list_tag);
    }
